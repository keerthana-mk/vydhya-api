...
class DoctorProfileRepository:
    ...
    def update_doctor_profile(user_id, user_profile_details):
        try:
            DoctorProfileRepository.database.query(DoctorProfile).filter(DoctorProfile.user_id == user_id).update(user_profile_details)
            DoctorProfileRepository.database.commit()
        except Exception as e:
            raise BaseException(e)
...
class InsurerProfileRepository:
    ...
    def get_insurer_profile(user_id):
        insurer_query = InsurerProfileRepository.database.query(InsurerProfile).filter((InsurerProfile.user_id == user_id))
    ...
    def update_insurer_profile(user_id, user_profile_details):
        try:
            InsurerProfileRepository.database.query(InsurerProfile).filter(InsurerProfile.user_id == user_id).update(user_profile_details)
            InsurerProfileRepository.database.commit()
        except Exception as e:
            raise BaseException(e)